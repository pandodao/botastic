...
var (
	limitChan = make(chan struct{}, 30)
)

var tmpl = template.Must(template.New("tmpl").Parse(`Web search results:
{{range .Items}}
[{{.Index}}] "{{.Snippet}}"
URL: {{.URL}}
{{end}}
...
func WebSearch(ctx context.Context, q string, count int) (*Result, error) {
	limitChan <- struct{}{}
